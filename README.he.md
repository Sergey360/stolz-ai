...
הפרויקט פועל לפי אותו עיקרון: **כל טוקן צריך לבצע עבודה מועילה**.
...
### ראיות למספר סביבות ב-v0.7

ראיות C2 תקפות רק עבור Claude Code 2.1.251 ו-Qwen Code 0.22.3 עם מתאם 1.0.0. ארבע הרצות מקומיות אומתו עם Codex CLI 0.153.4, אך ל-Codex אין רשומת C2 של v0.7. מנגנון הקבלה בזוגות של C3 נכשל באופן סגור, וכל זוגות הספקים הנוכחיים נשארים `withheld`. ההתקנה אינה מפעילה שירות רקע, מטמון משותף, מאגר מצב מתמשך או בקר סקרים אוטומטי.
...
| [`stolz-context`](skills/stolz-context/SKILL.md) | כשצריך לאמת מניפסט הקשר קיים של STOLZ או רשומה ביומן קריאות |
| [`stolz-reuse`](skills/stolz-reuse/SKILL.md) | כשתוצאה קודמת או פקודה זהה שכבר רצה אכן קיימות |
...
כשהצורך ברור, בחרו ישירות במיומנות המתאימה. קריאה ראשונה רגילה אינה מפעילה את
`stolz-context`, ובהיעדר תוצאה קודמת לא נבחרת `stolz-reuse`. אבחון v0.12 עם
