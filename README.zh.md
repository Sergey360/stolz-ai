...
C2 证据仅适用于使用 1.0.0 适配器的 Claude Code 2.1.251 和 Qwen Code 0.22.3。Codex CLI 0.153.4 已完成四次本地安装运行，但没有 v0.7 C2 记录。C3 成对准入按 fail-closed 方式实现，目前所有提供商组合仍为 `withheld`。安装不会启动后台服务、共享缓存、持久状态存储或自动轮询控制器。
