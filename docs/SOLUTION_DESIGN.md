...
| --- | --- | --- |
| `ADR-0001` | Use a provider-neutral skill suite with opt-in adapters. | Skills remain portable; adapter capability gaps use safe fallback routes. |
| `ADR-0002` | Put waiting, retries, cursors, and deduplication in deterministic helpers. | The model sees material events instead of unchanged-state heartbeats. |
| `ADR-0003` | Publish efficiency claims only from outcome-gated, reproducible comparisons. | Lower token counts cannot justify weakened correctness or verification. |
| `ADR-0004` | Treat production as immutable public release delivery, not a hosted service. | Release evidence and reversible replacement releases replace live-service smoke checks. |
| `ADR-0005` | Resolve profile-selected adapters and integrations only after declared triggers. | Profiles remain provider-neutral, initial context stays measurable, and unknown/global adapters are rejected. |
